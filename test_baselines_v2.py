...
class Tester:
    ...
    def output_results(self, labels, preds, probs, metrics):
        ...
        #     i = 0
        #     fpr, tpr, threshold = roc_curve((labels == i).astype(int), probs[:, i])
        #     # 写入 fpr tpr 到文件
        #     roc_file = os.path.join(self.output_dir, f"roc_curve_{self.model_type}.csv")
        #     with open(roc_file, "w") as f:
        #         f.write("FPR,TPR,Threshold\n")
        #         for fp, tp, th in zip(fpr, tpr, threshold):
        #             f.write(f"{fp},{tp},{th}\n")
        # else:
        # 算 macro avg
        ...
        # if self.num_classes == 2:
        #     i = 0
        #     precision, recall, thresholds = precision_recall_curve((labels == i).astype(int), probs[:, i])
        #     # 写入 fpr tpr 到文件
        #     pr_file = os.path.join(self.output_dir, f"pr_{self.model_type}.csv")
